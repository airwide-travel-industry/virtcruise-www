...
const airwideZip = `${root}dist/virtcruise-www-0.8.0-beta.1.zip`;
...
test('Airwide build preserves branded artifact and contains only Airwide runtime origins', async () => {
  ...
  const runtime = unzip('virtcruise-www-0.8.0-beta.1/js/runtime-config.js');
  const manifest = JSON.parse(unzip('virtcruise-www-0.8.0-beta.1/DEPLOYMENT-MANIFEST.json'));
  ...
  assert.equal(manifest.release, '0.8.0-beta.1');
