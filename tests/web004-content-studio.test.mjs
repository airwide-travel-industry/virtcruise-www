...
const api = await readFile('js/content-studio-api.js', 'utf8');
...
test('Content Studio exposes the editorial modules and uses existing management routes', () => {
  ...
  assert.match(api, /\/api\/v1\/content\/packages/);
  assert.match(api, /transition/);
  for (const command of ['submit', 'approve', 'reject', 'schedule', 'publish', 'retire', 'restore']) assert.match(script, new RegExp(command));
  for (const command of ['pricing', 'media', 'audit']) assert.match(api, new RegExp(command));
