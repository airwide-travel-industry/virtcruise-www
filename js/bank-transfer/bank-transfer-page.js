...
async function renderHome() {
  const [invoices, reviews] = await Promise.all([repository.invoices(), repository.reviews()]);
  const currency = list(invoices).find(item => Number(item.outstanding?.amount || 0) > 0)?.outstanding?.currency;
  const instructions = currency ? await repository.instructions(currency) : null;
  ...
  setPage(`${pageHeading('Secure payment option', 'Pay by Bank Transfer', 'Transfer funds using the authoritative details below, then send proof for independent Finance verification.', `<a class="portal-button" href="${portalUrl('/bank-transfer/new/')}">Create review case</a>`)}
    ...
    ${instructions ? instructionPanel(instructions) : '<section class="portal-panel"><h2>Bank instructions</h2><p>Authoritative bank details are shown after an outstanding invoice is available.</p></section>'}
...
function instructionPanel(instructions) { return `<section class="portal-panel" data-bank-instructions><p class="portal-eyebrow">Authoritative bank instructions</p><h2>${escapeHtml(instructions.displayName || instructions.bankName)}</h2><dl class="instruction-grid">${[['Bank',instructions.bankName],['Account name',instructions.accountName],['Account number',instructions.accountNumber],['Branch code',instructions.branchCode],['SWIFT',instructions.swift || 'Not required'],['Currency',instructions.currency]].map(([k,v])=>`<div><dt>${k}</dt><dd>${escapeHtml(v)}</dd></div>`).join('')}</dl><p>${escapeHtml(instructions.customerInstructions)}</p><p class="financial-data-note">Effective ${formatDate(instructions.effectiveFrom)}${instructions.effectiveTo?` until ${formatDate(instructions.effectiveTo)}`:''}. Your exact transfer reference is generated from the invoice you select.</p></section>`; }
...
async function renderNew() {
  ...
  const update = async () => { const invoice=available[Number(select.value)],submit=form.querySelector('[type=submit]');submit.disabled=true;box.hidden=!invoice;amount.disabled=true;if(invoice){amount.max=invoice.outstanding.amount;amount.value=invoice.outstanding.amount;try{instructions=await repository.instructions(invoice.outstanding.currency);document.querySelector('[data-selected-instructions]').innerHTML=instructionPanel(instructions);box.querySelector('[data-reference-value]').textContent=referenceFor(instructions.referencePrefix,invoice,sequenceFor(invoice));box.querySelector('[data-reference-rules]').textContent=instructions.referenceRules;amount.disabled=false;submit.disabled=false;}catch(error){announce(error.message);}}else{instructions=null;document.querySelector('[data-selected-instructions]').innerHTML='';amount.value='';} };
