...
async function renderNew() {
  ...
  const select = form.elements.invoice; const box = form.querySelector('[data-reference]');
...
async function renderDetails() {
  ...
  const rejected=String(review.reviewStatus).toUpperCase()==='REJECTED'||proofs[0]?.status==='REJECTED';
  setPage(`${pageHeading('Customer-safe review','Bank Transfer Progress','Finance verifies funds independently before payment and booking progression.')}
    ...
    <aside class="portal-panel"><p class="portal-eyebrow">Proof of payment</p><h2>${proofs.length?(rejected?'Replacement needed':'Your proof has been received.'):'Upload proof'}</h2>${rejected?`<div class="proof-warning" role="alert"><strong>Proof rejected</strong><p>${escapeHtml(safeReason(review.decisionReason))}</p></div>`:''}<form id="proofForm" class="upload-form"><label for="proofFile">${proofs.length?'Upload replacement proof':'Choose proof file'}</label><input id="proofFile" name="file" type="file" accept="application/pdf,image/jpeg,image/png" required><small>PDF, JPEG or PNG. The file is sent directly and is never stored by your browser.</small><button class="portal-button" type="submit">Upload proof</button><progress hidden max="1" aria-label="Uploading proof"></progress></form><div data-upload-state role="status" aria-live="polite"></div></aside></div>
