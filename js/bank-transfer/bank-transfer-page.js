...
function instructionPanel(instructions, invoice) { return `<section class="portal-panel" data-bank-instructions><p class="portal-eyebrow">Payment destination</p><h2>${escapeHtml(instructions.bankName)}</h2><dl class="instruction-grid">${[['Bank',instructions.bankName],['Account name',instructions.accountName],['Account number',instructions.accountNumber],['Branch code',instructions.branchCode],['SWIFT/BIC',instructions.swift || 'Not required'],['Currency',instructions.currency],...(invoice?[]:[['Payment reference',instructions.customerReference]]),['Amount due',formatMoney(instructions.amountDue,instructions.currency)]].map(([k,v])=>`<div><dt>${k}</dt><dd>${escapeHtml(v)}</dd></div>`).join('')}</dl><button type="button" class="portal-button secondary" data-copy-account>Copy account number</button><button type="button" class="portal-button secondary" data-copy-reference>Copy reference</button></section>`; }
