...
async function mutate(action) {
  ...
  try {
    ...
    if(['approve','reject'].includes(action)){const reason=await decisionDialog(action,review); if(!reason){mutationKeys.delete(intent);return;} await repository[action](review.id,reason,key);}
