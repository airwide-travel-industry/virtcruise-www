...
function invoicePanel(quote, linkedInvoice, bankAccounts = [], assignment = null, instruction = null) {
  if (linkedInvoice) {
    ...
    const destination = linkedInvoice.status === 'DRAFT' ? (eligible.length ? `<section class="payment-destination"><h3>Payment Destination</h3><label>Receiving Bank Account<select data-bank-assignment><option value="">Select account</option>${eligible.map(account => `<option value="${account.bankAccountId}" ${assignment?.bankAccountId === account.bankAccountId ? 'selected' : ''}>${escapeHtml(account.displayName)} · ${escapeHtml(account.bankName)} · ${escapeHtml(account.currency)} · ${maskedAccount(account.accountNumber)}</option>`).join('')}</select></label><button class="portal-button secondary" type="button" data-save-bank-account>Save</button></section>` : `<section class="payment-destination"><p>No active ${escapeHtml(linkedInvoice.total.currency)} receiving account is configured.</p><a class="portal-button secondary" href="/finance/bank-accounts/">Manage Bank Accounts</a></section>`) : instruction ? `<section class="payment-destination"><h3>Payment Instructions</h3><dl><div><dt>Bank</dt><dd>${escapeHtml(instruction.bankName)}</dd></div><div><dt>Account Name</dt><dd>${escapeHtml(instruction.accountName)}</dd></div><div><dt>Account</dt><dd>${maskedAccount(instruction.accountNumber)}</dd></div><div><dt>Branch</dt><dd>${escapeHtml(instruction.branchCode)}</dd></div><div><dt>SWIFT/BIC</dt><dd>${escapeHtml(instruction.swift || 'Not required')}</dd></div><div><dt>Currency</dt><dd>${escapeHtml(instruction.currency)}</dd></div><div><dt>Payment Reference</dt><dd>${escapeHtml(instruction.customerReference)}</dd></div></dl></section>` : linkedInvoice.status === 'ISSUED' && eligible.length ? `<section class="payment-destination"><h3>PAYMENT INSTRUCTIONS</h3><p>No payment instructions have been issued for this invoice.</p><label>Receiving Bank Account:<select data-legacy-bank-account><option value="">Select ACTIVE matching-currency account</option>${eligible.map(account => `<option value="${account.bankAccountId}">${escapeHtml(account.displayName)} · ${escapeHtml(account.bankName)} · ${escapeHtml(account.currency)} · ${maskedAccount(account.accountNumber)}</option>`).join('')}</select></label><button class="portal-button" type="button" data-issue-payment>Issue Payment Instructions</button></section>` : linkedInvoice.status === 'ISSUED' ? `<section class="payment-destination"><p>No active ${escapeHtml(linkedInvoice.total.currency)} receiving account is configured.</p><a class="portal-button secondary" href="/finance/bank-accounts/">Manage Bank Accounts</a></section>` : '';
    ...
    if (status(quote.status) === 'ACCEPTED') {
      ...
      if (linkedInvoice) {
        if (linkedInvoice.status === 'DRAFT') {
          ...
          try { instruction = await adminQuotes.paymentInstructions(linkedInvoice.id); } catch (error) { if (!(error instanceof AuthError && error.status === 404)) throw error; bankAccounts = collection(await adminQuotes.bankAccounts()); }
        }
      }
    }
...
function bindInvoice(id, quote, linkedInvoice) {
  ...
  document.querySelector('[data-issue-payment]')?.addEventListener('click', async event => {
    ...
    const bankAccountId = select?.value;
    if (!bankAccountId) { if (message) message.textContent = 'Select an ACTIVE matching-currency receiving account.'; return; }
    const account = select.selectedOptions[0]?.textContent || 'selected account';
    if (!window.confirm(`Issue bank-transfer instructions for this existing invoice?\n\nInvoice: ${linkedInvoice.number}\nAmount: ${linkedInvoice.total.currency} ${money(linkedInvoice.total.amount)}\nReceiving account: ${account}\n\nThis will create immutable payment instructions for this invoice.`)) return;
    event.currentTarget.disabled = true;
    try { await adminQuotes.assignBankAccount(linkedInvoice.id, bankAccountId); await loadDetail(id); }
    catch (error) { if (message) message.textContent = errorText(error); event.currentTarget.disabled = false; }
  });
