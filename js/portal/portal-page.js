...
function renderAdminDashboard() {
  setPage(`${pageHeading('ADMINISTRATION', 'Administration Dashboard', 'Manage Virtcruise administration and financial operations.')}
    <section class="dashboard-grid" aria-label="Administration operations">
      ...
      <a class="dashboard-card" href="${portalUrl('/operations-preview/')}" ><span aria-hidden="true">◈</span><strong>Operations <em>v0.8 Preview</em></strong><small>Manage assignments, incidents, change requests and operational readiness.</small><span>Preview Operations →</span></a>
    </section>`);
}
...
async function renderQuoteDetails() {
  ...
  setPage(`${pageHeading('Quote details', quote.quoteNumber || 'Your quote', quoteDestination(quote), `${acceptAction}<button class="portal-button secondary" type="button" data-print>Print quote</button>`)}
    ...
    <div class="detail-layout"><div>
      ...
      ${serviceGroups.map(([label, types]) => detailSection(label, items.filter(item => types.includes(String(item.type || item.serviceType).toUpperCase())), item => `<article class="detail-item"><div>${statusBadge(item.type || item.serviceType || 'SERVICE')}<strong>${escapeHtml(item.description || item.serviceTitle || item.title || 'Travel service')}</strong></div><p>${escapeHtml(item.notes || '')}</p></article>`)).join('')}
      ${detailSection('Other travel services', items.filter(item => !serviceGroups.some(([, types]) => types.includes(String(item.type || item.serviceType).toUpperCase()))), item => `<article class="detail-item"><div>${statusBadge(item.type || item.serviceType || 'SERVICE')}<strong>${escapeHtml(item.description || item.serviceTitle || item.title || 'Travel service')}</strong></div><p>${escapeHtml(item.notes || '')}</p></article>`)}
